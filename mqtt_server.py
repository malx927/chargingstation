...
from chargingorder.utils import uchar_checksum, byte2integer, get_byte_daytime, \
    get_byte_version, get_datetime_from_byte, message_escape, save_charging_cmd_to_db, \
...
def get_data_nums(byte_msg):
    ...
    """
    b_nums = byte_msg[1:3]
    nums = int.from_bytes(b_nums, byteorder="big")
    return nums
...
def get_pile_sn(byte_msg):
    ...
    """
    if len(byte_msg) > 16:
        pile_sn = byte_msg[2:10].decode('utf-8').strip('\000')
        return pile_sn
    else:
        return None


def get_32_byte(data, b_len=8):
...
def on_message(client, userdata, msg):
    ...
    if len(msg.payload) <= 16:
        logging.warning("长度小于16：{}".format(msg.payload))
        return
...
def pile_request_device_info(device_sn, byte_msg):
    ...
    byte_msg = byte_msg[3:]
    # sn号
    req_device_sn = byte_msg[1:9].decode('utf-8').strip('\000')

    logging.info("充电桩Sn编码:{}, sn：{}".format(pile_sn, req_device_sn))

    # 回复电桩
    data = {
        "pile_sn": pile_sn,
        "device_sn": req_device_sn,
    }
...
# 0x0a
def pile_request_user_balance(device_sn, byte_msg):
    """
    充电桩查询余额
    ...
    byte_msg = byte_msg[3:]
    # 用户标识
    user_flag = byte_msg[1]
    cardnum_ordernum = byte_msg[2:36].decode('utf-8').strip('\000')
    password = byte_msg[36:54].decode('utf-8').strip('\000')
...
def server_response_user_balance(*args, **kwargs):
    ...
    b_cardnum_ordernum = get_32_byte(cardnum_ordernum, b_len=34)
...
# 0x09
def pile_upload_offline_bill(device_sn, byte_msg):
    """
    充电桩上报离线结束充电产生的帐单
...
def server_reply_offline_bill(*args, **kwargs):
    ...
    b_out_trade_no = get_32_byte(out_trade_no, b_len=0)
    b_blank = bytes(14)
...
# 0x08
def pile_request_stage_tariff(device_sn, byte_msg):
    """充电桩请求电价信息"""
...
# 83(0x83)
def pile_card_charging_request_hander(device_sn, byte_msg):
    """桩端向后台请求充电"""
    logging.info("0x83 Enter pile_card_charging_request_hander")
    pile_sn = device_sn
    logging.info("充电桩Sn编码:{}".format(pile_sn))
    byte_msg = byte_msg[3:]
    card_type = byte_msg[1]
    logging.info('用户识别卡方式: {}'.format(card_type))
    # 保留四位
    gun_num = byte_msg[6]   # 枪口号
    logging.info("枪口号:{}".format(gun_num))

    oper_type = byte_msg[7]    # 操作类型(01:请求后台充电,02:请求后台停充)
    logging.info("操作类型：{}".format(oper_type))

    card_num = byte_msg[8:40].decode('utf-8').strip('\000')
    logging.info('卡号: {}'.format(card_num))

    cur_time = datetime.datetime.now().date()

    password = byte_msg[40:56].decode('utf-8').strip('\000')
...
# 1 (0x01)
def pile_status_handler_v12(device_sn, byte_msg):
    """
    ...
    # 工作状态枪1 and 工作状态枪 2
    gun1_status = byte_msg[7]  # 00空闲，01：故障， 10充电，11充电结束未拔枪
    gun1_desc = byte_msg[8]        # D7-D0位（低字节） 故障、充电、预约阶段代码
    logging.info('工作状态枪1:{}[{}]'.format(gun1_status, gun1_desc))
    # 枪1工作状态代码详细信息9 + 8

    gun2_status = byte_msg[17]
    gun2_desc = byte_msg[18]       # D5-D0位 故障、充电、预约阶段代码
...
def server_reply_version_info(*arg, **kwargs):
    ...
    b_current_time = get_byte_daytime(datetime.datetime.now())
...
def server_send_charging_cmd(*args, **kwargs):
    ...
    b_out_trade_no = get_32_byte(out_trade_no, b_len=32)
...
# 8 (0x04)
def pile_reply_charging_cmd_handler(device_sn, byte_msg):
    """
    ...
    # 订单
    out_trade_no = byte_msg[2:34].decode('utf-8').strip('\000')
    logging.info("订单号:{}".format(out_trade_no))
    # 充电状态
    charg_status = byte_msg[34]
    begin_reading = byte2integer(byte_msg, 35, 39)
    # 保留（25字节）
...
# 9(0x05)----> 通知用户
def pile_report_car_info_handler(device_sn, byte_msg):
    """
    接收并处理充电桩上报的充电车辆信息
    9、开始充电作业
    ...
    logging.info("VIN码:{}".format(byte_msg[35:52].hex()))

    max_single_voltage = byte2integer(byte_msg, 52, 54)
    # 车辆最高充电电压
    max_voltage = byte2integer(byte_msg, 54, 56)
    # 车辆最高充电电流
    max_current = byte2integer(byte_msg, 56, 58)
    max_temp = byte_msg[58]

    begin_soc = byte_msg[59]
...
# 11(0x06)
def pile_charging_status_handler(device_sn, byte_msg):
    """
    ...
    """
    logging.info("0x06 Enter pile_charging_status_handler")
    data_nums = get_data_nums(byte_msg)
    # 读取电桩编码(sn)
    pile_sn = device_sn
    byte_msg = byte_msg[3:]
    ...
    # 截止时间
    b_charg_time = byte_msg[34:41]
    charg_time = get_datetime_from_byte(b_charg_time)
    # 当前SOC
    current_soc = byte_msg[41]
    logging.info('充电时间:{},当前SOC:{}'.format(charg_time, current_soc))
    # # 所需电压值
    voltage = byte2integer(byte_msg, 42, 44)  # （备注：交流无此值）
    # 所需电流值
    current = byte2integer(byte_msg, 44, 46)  # （备注：交流无此值）
    # 输出电压值
    output_voltage = byte2integer(byte_msg, 46, 48)
    # 输出电流值
    output_current = byte2integer(byte_msg, 48, 50)
    # 枪头温度值
    gun_temp = byte_msg[50]
    gun_temp1 = byte_msg[51]
    # 柜内温度值
    cab_temp = byte_msg[52]
    cab_temp1 = byte_msg[53]
    # 当前电表读数
    current_readings = byte2integer(byte_msg, 54, 58)
...
# 13 (0x07)
def pile_charging_stop_handler(device_sn, byte_msg):
    """
    13、充电桩停止充电回复或主动上报
    ...
    # 当前SOC
    current_soc = byte_msg[34]
    # 当前电表读数
    current_reading = int.from_bytes(byte_msg[35:39], byteorder="big")
    # 停止充电回复代码 0、已充满，1后台中止，2车端停止，3故障停止 4、已中止的帐单或充电机处于空闲
    stop_code = byte_msg[39]
    # 运行状态码
    state_code = byte_msg[40]
    # 故障代码
    fault_code = byte_msg[41]
